  ...
  public function load($name) {
    ...
    if($file) {
      ...
      if (pathinfo($file, PATHINFO_EXTENSION) === 'php') {
        // discard any output generated by legacy PHP blueprints that just contain YAML since
        // we expect PHP logic blueprints to return an array in Kirby 2.5+
        ob_start();
        $this->yaml = require($this->file);
        ob_end_clean();

        if (!is_array($this->yaml)) {
          throw new Exception(l('blueprints.error.missing.logic.array'));
        }
      } else {
